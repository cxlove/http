...
COMMON_OBJS=$(OBJ_DIR)/http_request.o $(OBJ_DIR)/http_response.o $(OBJ_DIR)/http_method.o $(OBJ_DIR)/http_protocol.o $(OBJ_DIR)/http_status.o $(OBJ_DIR)/http.o  $(OBJ_DIR)/socklib.o
SERVER_MAIN=$(OBJ_DIR)/http_server_main.o 
CLIENT_MAIN=$(OBJ_DIR)/http_client_main.o
SOCKLIB_MAIN=$(OBJ_DIR)/socklib_main.o
...
SOCKLIB_BIN=$(BIN_DIR)/socklib_main

all: chkobjdir chkbindir $(SERVER_BIN) $(CLIENT_BIN) $(SOCKLIB_BIN)
...
$(SOCKLIB_BIN): $(OBJS) $(SOCKLIB_MAIN)
	$(CXX) -o $@ $(OBJS) $(SOCKLIB_MAIN) $(CPPFLAGS)
