/*************************************************************************
	...
	> Created Time: 日  3/13 12:05:36 2016
 ************************************************************************/
...
bool HttpRequest::Parse(void) {
    ...
    if(next_index == std::string::npos) {
        LOG(ERROR) << "Parse: Parse http request failed, the \
            request method is not impletmented.";
        return false;
    }
    ...
    while(true) {
        ...
        next_index = data_.find_first_of(CRLF, now_index);
        if (next_index == std::string::npos) {
            LOG(ERROR) << "Parse: Parse http request failed, \
                invalid header.";
            return false;
        }
