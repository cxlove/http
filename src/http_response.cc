/*************************************************************************
	...
	> Created Time: 一  3/14 20:24:08 2016
 ************************************************************************/
...
bool HttpResponse::Parse(void) {
    ...
    if(next_index == std::string::npos) {
        LOG(ERROR) << "Parse: Parse http response failed, the \
            response protocol is not impletmented.";
        return false;
    }
    ...
    if(next_index == std::string::npos) {
        LOG(ERROR) << "Parse: Parse http response failed, the \
            status code is not impletmented.";
        return false;
    }
    ...
    if(next_index == std::string::npos) {
        LOG(ERROR) << "Parse: Parse http response failed, the \
            reason phrase is not impletmented.";
        return false;
    }
    ...
    while(true) {
        ...
        next_index = data_.find_first_of(CRLF, now_index);
        if (next_index == std::string::npos) {
            LOG(ERROR) << "Parse: Parse http response failed, \
                invalid header.";
            return false;
        }
        std::string header = data_.substr(now_index, next_index - now_index);
        int mid_index = data_.find_first_of(':', now_index);
        if(mid_index == std::string::npos || mid_index > next_index) {
            LOG(ERROR) << "Parse: Parse http response failed, \
                invalid header, no \':\' between name and content.";
            return false;
        }
